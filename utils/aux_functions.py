...
def get_line(face_landmark, image, type="eye", debug=False):
    ...
    left_eye_mid = np.mean(np.array(left_eye), axis=0)  # return a center point of left eye
...
def get_six_points(face_landmark, image):
    ...
    face_b = m
    #face_b = (face_landmark['nose_tip'][2][0] +  face_landmark['top_lip'][3][0])//2, \
    #         (face_landmark['nose_tip'][2][1] +  face_landmark['top_lip'][3][1])//2
    #face_b = face_landmark['top_lip'][9]
...
def mask_face(image, face_location, six_points, angle, args, type="surgical"):
    ...
    face_height = face_location[2] - face_location[0]  #size of face
    ...
    delta_b = 1 + 2*(img_brightness - mask_brightness) / 255
