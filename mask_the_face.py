...
if args.new_path !='':
    args.write_path = args.new_path + "_masked"
...
mask_code = "".join(args.code.split()).split(",") # code split
...
if args.path.endswith('.txt'):
    ...
    # for line in lines:
        ...
        #     )
        #     for i in range(len(mask)):
        #         w_path = write_path.replace('asd/','asd_pro/') + "_" + mask[i] + "." + line.rsplit(".")[1]
        #         print(w_path)
    ...
    for f in tqdm(lines):
        ...
        # print(f)
        image_path = f
        write_path = os.path.dirname(f)
        write_path = write_path.replace('asd','asd_asd')
        # print(write_path)
        if not os.path.isdir(write_path):
            os.makedirs(write_path)
        ...
        if is_image(image_path):
            ...
            split_path = os.path.basename(f).rsplit(".")
            masked_image, mask, mask_binary_array, original_image = mask_image(
                image_path, args
            )
            for i in range(len(mask)):
